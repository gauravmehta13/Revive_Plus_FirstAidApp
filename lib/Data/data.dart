var hands = [
  {
    "title": "Hand injuries",
    "subtitle":
        "Pain in Hands is more than just annoying. The stiffness and swelling that go along with hand pain can sap strength and diminish the ability to carry out routine functions, like buttoning clothes.",
    "imgurl": "assets/hand.jpg",
    "h1": "• Protection ",
    "sh1":
        "In this context, it means to remove the person from task to protect against further injury. This is especially important in the first 48 hours after the injury occurred. ",
    "h2": "• Rest ",
    "sh2":
        "The person should not continue with any sporting activity following a hand injury",
    "h3": "• Ice ",
    "sh3":
        "The aim of applying ice is to relieve pain. 20 minutes with an ice pack every other hour for a day or two generally has a good effect. Even though there are many commercial ice products available, the best solution is often a plastic bag filled with crushed ice and some water. Place a damp towel between the ice pack and skin. ",
    "h4": "• Compression ",
    "sh4":
        "Following a finger injury, the most important thing is to apply a pressure bandage. Applying compression minimises swelling, which in turn may decrease stiffness. Use an elastic bandage that is small enough for this type of injury. Sports tape is not advised, due to lack of elasticity. The use of a pressure bandage should be continued for the first 2-3 days. ",
    "h5": "• Elevation ",
    "sh5":
        "Swelling can be reduced further by keeping the hand elevated, preferably above heart level. This is particularly important in the first few hours, but it is best to continue to keep it elevated as much as possible for the first 24 hours. Remember that compression should be maintained around the clock to keep internal bleeding (swelling) to a minimum.",
  },
];

var legs = [
  {
    "title": "Injury in Legs?",
    "subtitle":
        "Your legs support your body and allow you to walk, run, and jump. But like any other part of your body, they can get hurt or wear out.",
    "imgurl": "assets/leg.jpg",
    "h1": "• Rest",
    "sh1":
        "Rest the injured limb. Your doctor may recommend not putting any weight on the injured area for 48 to 72 hours, so you may need to use crutches. A splint or brace also may be helpful initially. But don't avoid all activity.Even with an ankle sprain, you can usually still exercise other muscles to minimize deconditioning. For example, you can use an exercise bicycle with arm exercise handles, working both your arms and the uninjured leg while resting the injured ankle on another part of the bike. That way you still get three-limb exercise to keep up your cardiovascular conditioning.",
    "h2": "• Ice",
    "sh2":
        "Ice the area. Use a cold pack, a slush bath or a compression sleeve filled with cold water to help limit swelling after an injury. Try to ice the area as soon as possible after the injury and continue to ice it for 15 to 20 minutes, four to eight times a day, for the first 48 hours or until swelling improves. If you use ice, be careful not to use it too long, as this could cause tissue damage.",
    "h3": "• Compression",
    "sh3":
        "Compress the area with an elastic wrap or bandage. Compressive wraps or sleeves made from elastic or neoprene are best.",
    "h4": "• Elevation",
    "sh4":
        "Elevate the injured limb above your heart whenever possible to help prevent or limit swelling.",
    "h5": "",
    "sh5": "",
  },
];

var face = [
  {
    "title": "facial Injuries",
    "subtitle": "Prevention is the best policy, but accidents do happen",
    "imgurl": "assets/face.jpg",
    "h1": "• Facial Bleeding",
    "sh1":
        "  • Cover Wound with cloth.\n  • Apply pressure. \n  • Do not remove soaked cloth \n  • Use additional Cloth and continue Pressure",
    "h2": "• Nasal Fracture",
    "sh2":
        "  • Gently pack nose with gauze.\n  • Apply Ice.\n  • Do not Blow nose.",
    "h3": "• Jaw Fracture",
    "sh3":
        "  • Gently align jaws (Do not use force).\n  • Immobilize Jaw by wrapping a bandage under the chin.\n  • Apply Ice to control swelling and Bleeding.\n  • Seek emergency care.\n  • Consult an Oral and maxillofacial surgeon immidiately.",
    "h4": "• Broken Teeth ",
    "sh4":
        "  • Clean injured area.\n  • Apply ice.\n  • Save broken tooth.\n  • Bring to a general dentist for reattachment",
    "h5": "",
    "sh5": "",
  },
];

var chest = [
  {
    "title": "Chest Pain Treatment",
    "subtitle":
        "Causes of chest pain can vary from minor problems, such as indigestion or stress, to serious medical emergencies, such as a heart attack or pulmonary embolism. The specific cause of chest pain can be difficult to interpret.",
    "imgurl": "assets/chest.jpg",
    "h1": "• For Heart attack",
    "sh1":
        "• Call 911 or emergency medical assistance.\n• Chew a regular-strength aspirin.\n• Take nitroglycerin, if prescribed.\n• Begin CPR on the person having a heart attack, if directed.\n• If an automated external defibrillator (AED) is immediately available and the person is unconscious, follow the device instructions for using it.",
    "h2": "• For GERD (Acid Reflux)",
    "sh2":
        "• Treat With Over-the-Counter Antacids\n• Inform the person’s doctor about the chest pain and ER visit.",
    "h3": "• For Angina",
    "sh3":
        "• Treat With Nitroglycerin\nIf the person gets angina and has been prescribed nitroglycerin: Dissolve 1 nitroglycerin tablet under the tongue (or use nitroglycerin spray under the tongue).Wait 5 minutes.\nIf the person still has angina, call 911.",
    "h4": "",
    "sh4": "",
    "h5": "",
    "sh5": "",
  },
];

var abdomen = [
  {
    "title": "Abdominal pain",
    "subtitle":
        "Serious causes of abdominal pain include appendicitis and pregnancy problems. However, most abdominal pain is harmless and goes away without surgery.",
    "imgurl": "assets/abdominal.jpg",
    "h1": "• Call 911 if:",
    "sh1":
        "• The pain is in your lower right abdomen and tender to the touch, and you also have fever or are vomiting. These may be signs of appendicitis.\n\n• You're vomiting blood.\n• You have a hard time breathing.\n• You're pregnant and have belly pain or vaginal bleeding.",
    "h2": "• Home Remedies",
    "sh2":
        "• You might try a heating pad to ease belly pain. Chamomile or peppermint tea may help with gas. Be sure to drink plenty of clear fluids so your body has enough water.\n\n• Eat several smaller meals instead of three big ones\n• Chew your food slowly and well\n• Stay away from foods that bothers you.\n• Ease stress with exercise, meditation, or yoga.",
    "h3": "",
    "sh3": "",
    "h4": " ",
    "sh4": "",
    "h5": "",
    "sh5": "",
  },
];

var pelvis = [
  {
    "title": "Pelvic Pain",
    "subtitle":
        "Pelvic pain is a discomfort that occurs in the area below your belly button and between your hips that lasts for 6 months or longer. It could be caused by a number of conditions (reproductive, urinary, digestive), or it might have no cause at all. ",
    "imgurl": "assets/pelvis.jpg",
    "h1": "• Over-the-counter pain relievers.",
    "sh1":
        "Taking ibuprofen (Advil, Motrin) or acetaminophen (Tylenol) is a good first step for CPP relief. Take them as directed to reduce the swelling that can lead to chronic pelvic pain.",
    "h2": "• Get moving.",
    "sh2":
        "It might be hard to think about exercise when you feel you can’t get off the couch—but you must. Exercise increases blood flow. What’s more, when you work out, your body releases feel-good chemicals (doctors call these “endorphins.”) These are you body’s natural painkillers. Just 30 to 45 minutes of moderate exercise -- like speed walking -- 5 or 6 days a week can help to relieve your pain.",
    "h3": "• Take the heat.",
    "sh3":
        "It helps increase blood flow, which may help reduce your pain. Sit in a tub full of warm water to provide relief during flare-ups. If you don’t have a tub, then a heating pad or warm compress on your belly can work just as well.",
    "h4": "• Make a change.",
    "sh4":
        "Just tweaking some of your habits can have an effect on your pain. If you smoke, stop. Nicotine -- the active ingredient in tobacco products -- inflames nerves and triggers pain. If you’re overweight, shedding the extra pounds may help ease pressure on nerves and, in turn, relieve pain.",
    "h5": "• Relax",
    "sh5":
        "Meditation, yoga, and even deep breathing exercises can help to reduce the stress and tension that can make pelvic pain even worse. Another bonus: They might help you sleep better.",
  },
];

var head = [
  {
    "title": "Headache",
    "subtitle":
        "Headaches usually are brief and can be caused by many things, including too little sleep, eye strain, stress, sinus infections, or a bump to the head. Some headaches last longer and come with other symptoms. Very rarely, headaches can be a sign of something serious.",
    "imgurl": "assets/head.jpg",
    "h1": "• Take Pain Medication",
    "sh1":
        "Give ibuprofen (Advil, Motrin), aspirin, or acetaminophen (Tylenol) for pain. Avoid ibuprofen and other NSAIDs if the person has heart failure or kidney failure. Do not give aspirin to a child under age 18",
    "h2": "• Try Self-Care Strategies",
    "sh2":
        "• The person should drink plenty of liquids. Dehydration can cause or worsen a headache\n• Apply a cool cloth or ice pack to the forehead, temples, or back of the neck.\n• Massage the person's neck and back to relieve muscle tension.\n• Apply gentle, rotating pressure to the painful area of the head.\n• Have the person rest or take a warm bath or shower.",
    "h3": "• When to Call a Doctor",
    "sh3":
        "Seek medical care immediately for:\n   • A headache that occurs after a head injury\n   • A headache that is accompanied by dizziness, speech impairment, confusion, or other neurological symptoms\n   • A headache that gets worse even after taking pain medications",
    "h4": " ",
    "sh4": "",
    "h5": "",
    "sh5": "",
  },
];
var hip = [
  {
    "title": "Hip Pain",
    "subtitle":
        "If you or someone else is experiencing hip pain it will usually be due to an inflamed tendon, a fracture to one of the bones in the hip joint, arthritis, or (less commonly) a cartilage tear.",
    "imgurl": "assets/hip.jpg",
    "h1": "• Rest",
    "sh1":
        "• Take plenty of rest. Avoid any activity that may put pressure on the hip. Avoid sleeping on the affected side and do not sit for extended periods of time.",
    "h2": "• Ice",
    "sh2":
        "Apply ice packs wrapped in a towel over the affect hip side. Take warm baths or showers to prepare the muscles for stretching exercises that are generally recommended to minimize pain.",
    "h3": "• Elevation ",
    "sh3":
        "Swelling can be reduced further by keeping the thigh elevated, preferably above heart level. This is particularly important in the first few hours, but it is best to continue to keep it elevated as much as possible for the first 24 hours. Remember that compression should be maintained around the clock to keep internal bleeding (swelling) to a minimum.",
    "h4": "• Compression. ",
    "sh4":
        "We can use compression by wrapping a thick Ace bandage around the pelvis and hip.",
    "h5": "• Exercise for Hip Pain Relief",
    "sh5":
        "To protect your hip joint, these exercises are the preferred choices:\n   • Walking in a shallow pool\n   • Walking on a flat surface, whether it’s outside or on a treadmill\n   • Swimming (being sure to kick gently)\n   • Taking a bike ride or riding a stationary bike\n   • Strengthening your upper body",
  },
];

var back = [
  {
    "title": "Back Pain Treatment",
    "subtitle":
        "A common myth about back pain is that you need to rest and avoid activity for a long time. In fact, doctors DO NOT recommend bed rest. If you have no sign of a serious cause for your back pain (such as loss of bowel or bladder control, weakness, weight loss, or fever), stay as active as possible.",
    "imgurl": "assets/back.jpg",
    "h1": "• Self-Care at Home",
    "sh1":
        "• Sleeping with a pillow between the knees while lying on one side may increase comfort. Some doctors recommend lying on your back with a pillow under your knees.\n• No specific back exercises have been found that improve pain or increase functional ability in people with acute back pain. Exercise, however, may be useful for people with chronic back pain to help them return to normal activities and work.",
    "h2": "• Ice",
    "sh2":
        "Put an ice pack on the injured area for 20 minutes at a time, four to eight times a day. Use a cold pack, or fill a bag with ice and wrap it in a towel. Continue for 48 hours after the injury. During this period, icing the affected area decreases inflammation by constricting your blood vessels. This limits blood flow to the area. Don’t apply heat during this time. It will have the opposite effect.",
    "h3": "• Compression",
    "sh3":
        "Applying pressure also helps reduce swelling. To apply pressure, wrap an elastic bandage around the affected area of your back. It may be easier and less painful to ask someone else to wrap it for you. To avoid cutting off your blood circulation, don’t wrap it too tightly. Loosen the bandage if the pain increases, the wrapped area becomes numb, or you notice swelling below the wrapped area.",
    "h4": "• Medications",
    "sh4":
        "An over-the-counter nonsteroidal anti-inflammatory (NSAID) drug may help relieve pain and swelling. Examples of NSAIDs include:\n   • aspirin\n   • ibuprofen (Advil, Motrin)\n   • naproxen (Aleve, Naprosyn)\n",
    "h5": "",
    "sh5": "",
  },
];
var scapula = [
  {
    "title": "Scapula pain",
    "subtitle":
        "Scapula region pain is a very common experience, with causes ranging from simple strains to serious conditions, such as heart attacks or liver problems.",
    "imgurl": "assets/scapula.jpg",
    "h1": "• Exercise",
    "sh1":
        "Physical activity is important for overall health, but exercise can also strengthen areas in your back, which may help with pain. Pushups, pullups, and situps are good exercises to strengthen muscles in your back and abdomen.",
    "h2": "• Stretching",
    "sh2":
        "Yoga and other stretching techniques can improve circulation to your muscles and joints, which may offer better mobilityTrusted Source and pain relief.\n\nThis shoulder stretch sometimes helps:\n   •Cross one arm over your body.\n   •Use your other arm to pull the elbow of your outstretched arm toward your chest.\n   •Hold this stretch for about 10 seconds. ",
    "h3": "• Diet",
    "sh3":
        "Certain foods can causeTrusted Source inflammation in your body that could worsen your symptoms. Avoid processed foods, and eat plenty of fruits and vegetables. Choosing foods high in omega-3 fatty acids, such as salmon, may also help.",
    "h4": "• Rest",
    "sh4":
        "Sometimes, you just need rest to recover from your shoulder blade pain, especially if it’s the result of an injury.",
    "h5": "",
    "sh5": "",
  },
];

var burns = [
  {
    "title": "Burns and Scalds",
    "subtitle":
        "Burns and scalds are damage to the skin caused by heat. Both are treated in the same way.\n\nA burn is caused by dry heat – by an iron or fire, for example. A scald is caused by something wet, such as hot water or steam.",
    "imgurl": "assets/burn.jpg",
    "h1": "• First Aid Advise",
    "sh1":
        "   • Immediately get the person away from the heat source to stop the burning.\n   • cool the burn with cool or lukewarm running water for 20 minutes – don't use ice, iced water, or any creams or greasy substances such as butter\n   • remove any clothing or jewellery that's near the burnt area of skin, including babies' nappies - but don't move anything that's stuck to the skin\n   • make sure the person keeps warm – by using a blanket, for example, but take care not to rub it against the burnt area\n   • cover the burn by placing a layer of cling film over it – a clean plastic bag could also be used for burns on your hand\n   • use painkillers such as paracetamol or ibuprofen to treat any pain\n   • if the face or eyes are burnt, sit up as much as possible, rather than lying down - this helps to reduce swelling",
    "h2": "• Call 911 for:",
    "sh2":
        "   • all chemical and electrical burns\n   • large or deep burns – any burn bigger than your hand\n   • burns that cause white or charred skin – any size\n   • burns on the face, hands, arms, feet, legs or genitals that cause blisters",
    "h3": "",
    "sh3": "",
    "h4": "",
    "sh4": "",
    "h5": "",
    "sh5": "",
  },
];
var cuts = [
  {
    "title": "Cuts",
    "subtitle":
        "Many people get a cut from falls or using sharp objects like scissors. Most of the cuts can be safely treated at home.",
    "imgurl": "assets/cut.jpg",
    "h1": "• Stop Bleeding",
    "sh1":
        "   • Apply direct pressure on the cut or wound with a clean cloth, tissue, or piece of gauze until bleeding stops.\n   • If blood soaks through the material, don’t remove it. Put more cloth or gauze on top of it and continue to apply pressure.\n   • If the wound is on the arm or leg, raise limb above the heart, if possible, to help slow bleeding.\n   • Wash your hands again after giving first aid and before cleaning and dressing the wound.\n   • Do not apply a tourniquet unless the bleeding is severe and not stopped with direct pressure.",
    "h2": "• Clean Cut or Wound",
    "sh2":
        "   • Gently clean with soap and warm water. Try to rinse soap out of wound to prevent irritation.\n   • Don’t use hydrogen peroxide or iodine, which can damage tissue.",
    "h3": "• Protect the Wound",
    "sh3":
        "   • Apply antibiotic cream to reduce risk of infection and cover with a sterile bandage.\n   • Change the bandage daily to keep the wound clean and dry.",
    "h4": "• When to Call a Doctor",
    "sh4":
        "   • The wound is deep or the edges are jagged or gaping open.\n   • The wound is on the person’s face.\n   • The wound has dirt or debris that won’t come out.\n   • The wound shows signs of infection, such as redness, tenderness, or a thick discharge, or if the person runs a fever.\n   • The area around the wound feels numb.\n   • Red streaks form around the wound.\n   • The wound is a result of an animal or human bite.\n   • The person has a puncture wound or deep cut and hasn’t had a tetanus shot in the past five years, or anyone who hasn’t had a tetanus shot in the past 10 years.",
    "h5": "",
    "sh5": "",
  },
];
var abrasions = [
  {
    "title": "Skin Abrasions",
    "subtitle":
        "An abrasion is a type of open wound that’s caused by the skin rubbing against a rough surface. It may be called a scrape or a graze. When an abrasion is caused by the skin sliding across hard ground, it may be called road rash.",
    "imgurl": "assets/abrasion.jpg",
    "h1": "• Treatment",
    "sh1":
        "A first- or second-degree abrasion can usually be treated at home. To care for an abrasion:\n\n   • Begin with washed hands.\n   • Gently clean the area with cool to lukewarm water and mild soap. Remove dirt or other particles from the wound using sterilized tweezers.\n   • For a mild scrape that’s not bleeding, leave the wound uncovered.\n   • If the wound is bleeding, use a clean cloth or bandage, and apply gentle pressure to the area to stop any bleeding. Elevating the area can also help stop bleeding.\n   • Cover a wound that bled with a thin layer of topical antibiotic ointment, like Bacitracin, or a sterile moisture barrier ointment, like Aquaphor. Cover it with a clean bandage or gauze. Gently clean the wound and change the ointment and bandage once per day.\n   • Watch the area for signs of infection, like pain or redness and swelling. See your doctor if you suspect infection.",
    "h2": "• When to Call the Doctor",
    "sh2":
        "Most abrasions don’t need a doctor's care. But call your doctor if:\n\n   • bleeding doesn’t stop after at least five minutes of pressure\n   • bleeding is severe, or profuse\n   • a violent or traumatic accident caused the wound.\n\nSee a doctor immediately if you suspect your wound has become infected. Infections that are left untreated can spread and lead to much more serious medical conditions.",
    "h3": "",
    "sh3": "",
    "h4": "",
    "sh4": "",
    "h5": "",
    "sh5": "",
  },
];
var stings = [
  {
    "title": "Insect bites and stings",
    "subtitle":
        "Most reactions to insect bites and stings are mild, causing little more than redness, itching, stinging or minor swelling. Rarely, insect bites and stings, such as from a bee, a wasp, a hornet, a fire ant or a scorpion, can result in severe reactions. ",
    "imgurl": "assets/bite.jpg",
    "h1": "• For mild reactions",
    "sh1":
        "To take care of an insect bite or sting that causes a mild reaction:\n\n   • Move to a safe area to avoid more bites or stings.\n   • If needed, remove the stinger.\n   • Wash the area with soap and water.\n   • Apply a cool compress. Use a cloth dampened with cold water or filled with ice. This helps reduce pain and swelling. If the injury is on an arm or leg, elevate it.\n   • Apply 0.5 or 1 percent hydrocortisone cream, calamine lotion or a baking soda paste to the bite or sting several times daily until your symptoms go away.\n   • Take an antihistamine (Benadryl, others) to reduce itching.\n\nUsually, the signs and symptoms of a bite or sting disappear in a day or two. If you're concerned — even if your reaction is minor — call your doctor.",
    "h2": "• When to seek emergency care",
    "sh2":
        "Call 911 or your local emergency number if the injured person experiences:\n\n   • Difficulty breathing\n   • Swelling of the lips, eyelids or throat\n   • Dizziness, faintness or confusion\n   • Rapid heartbeat\n   • Hives\n   • Nausea, cramps or vomiting\n   • A scorpion sting and is a child\n\nTake these actions immediately while waiting for medical help:\n\n   • Ask the person if he or she is carrying an epinephrine autoinjector (EpiPen, Auvi-Q, others) to treat an allergic attack.\n   • If the person says he or she needs to use an autoinjector, ask whether you should help inject the medication. This is usually done by pressing the autoinjector against the person's thigh and holding it in place for several seconds.\n   • Loosen tight clothing and cover the person with a blanket. Don't give him or her anything to drink.\n   • If the person is vomiting, position him or her to prevent choking.\n   • Begin CPR if the person shows no signs of circulation, such as breathing, coughing or movement.",
    "h3": "",
    "sh3": "",
    "h4": "",
    "sh4": "",
    "h5": "",
    "sh5": "",
  },
];
var splinters = [
  {
    "title": "Splinters",
    "subtitle":
        "Splinters are objects that become embedded under the skin. Most often these are tiny pieces of wood, although glass, metal, and plastic can be splinters too.",
    "imgurl": "assets/splinters.jpg",
    "h1": "• What to Do",
    "sh1":
        "To remove a splinter:\n\n   • Wash your hands, then the area around the splinter with soap and warm water.\n   • Sterilize tweezers and a needle by putting the ends of each in boiling water, then wipe them off with a clean cotton ball or alcohol pad.\n   • If the splinter is sticking through the skin, firmly grip the end with the tweezers. Pull slowly and gently at the same angle the splinter went in so it won't break.\n   • If the tip of the splinter isn't sticking out, gently scrape the skin away from the splinter with the needle until there's an end to grab with the tweezers.\n   • After the splinter is removed, wash the area again and cover it with a bandage.",
    "h2": "• Get Medical Care if:",
    "sh2":
        "   • the splinter seems to be too deep or won't come out\n   • the splinter is under a fingernail or toenail and can't be reached\n   • the wound is bleeding a lot\n   • the skin around the wound looks infected\n   • your child's vaccines are not up to date",
    "h3": "",
    "sh3": "",
    "h4": "",
    "sh4": "",
    "h5": "",
    "sh5": "",
  },
];
var sprain = [
  {
    "title": "Sprain",
    "subtitle":
        "Your ligaments are tough, elastic-like bands that connect bone to bone and hold your joints in place. A sprain is an injury to a ligament caused by tearing of the fibers of the ligament. The ligament can have a partial tear, or it can be completely torn apart.",
    "imgurl": "assets/sprain.jpg",
    "h1": "• Rest",
    "sh1":
        "Rest the injured limb. Your doctor may recommend not putting any weight on the injured area for 48 to 72 hours, so you may need to use crutches. A splint or brace also may be helpful initially. But don't avoid all activity.\n\nEven with an ankle sprain, you can usually still exercise other muscles to minimize deconditioning. For example, you can use an exercise bicycle with arm exercise handles, working both your arms and the uninjured leg while resting the injured ankle on another part of the bike. That way you still get three-limb exercise to keep up your cardiovascular conditioning.",
    "h2": "• Ice",
    "sh2":
        "Ice the area. Use a cold pack, a slush bath or a compression sleeve filled with cold water to help limit swelling after an injury. Try to ice the area as soon as possible after the injury and continue to ice it for 15 to 20 minutes, four to eight times a day, for the first 48 hours or until swelling improves. If you use ice, be careful not to use it too long, as this could cause tissue damage.",
    "h3": "• Compress",
    "sh3":
        "Compress the area with an elastic wrap or bandage. Compressive wraps or sleeves made from elastic or neoprene are best.",
    "h4": "• Elevate",
    "sh4":
        "Elevate the injured limb above your heart whenever possible to help prevent or limit swelling.\n\n\nSprains can take days to months to recover. As the pain and swelling improve, gently begin using the injured area. You should feel a gradual, progressive improvement. Over-the-counter pain relievers, such as ibuprofen (Advil, Motrin IB, others) and acetaminophen (Tylenol, others), may be helpful to manage pain during the healing process.",
    "h5": "",
    "sh5": "",
  },
];
var strain = [
  {
    "title": "Strain",
    "subtitle":
        "A strain is when a muscle is stretched too much and tears. It is also called a pulled muscle. A strain is a painful injury. It can be caused by an accident, overusing a muscle, or using a muscle in the wrong way.",
    "imgurl": "assets/strain.jpg",
    "h1": "• Rest",
    "sh1":
        "Rest the injured limb. Your doctor may recommend not putting any weight on the injured area for 48 to 72 hours, so you may need to use crutches. A splint or brace also may be helpful initially. But don't avoid all activity.\n\nEven with an ankle sprain, you can usually still exercise other muscles to minimize deconditioning. For example, you can use an exercise bicycle with arm exercise handles, working both your arms and the uninjured leg while resting the injured ankle on another part of the bike. That way you still get three-limb exercise to keep up your cardiovascular conditioning.",
    "h2": "• Ice",
    "sh2":
        "Ice the area. Use a cold pack, a slush bath or a compression sleeve filled with cold water to help limit swelling after an injury. Try to ice the area as soon as possible after the injury and continue to ice it for 15 to 20 minutes, four to eight times a day, for the first 48 hours or until swelling improves. If you use ice, be careful not to use it too long, as this could cause tissue damage.",
    "h3": "• Compress",
    "sh3":
        "Compress the area with an elastic wrap or bandage. Compressive wraps or sleeves made from elastic or neoprene are best.",
    "h4": "• Elevate",
    "sh4":
        "Elevate the injured limb above your heart whenever possible to help prevent or limit swelling.",
    "h5": "• When to see a doctor",
    "sh5":
        "For mild to moderate strains, home treatment should be enough. Seek medical attention if any of the following happens:\n\n   • The pain doesn’t subside after a week.\n   • The injured area is numb.\n   • There’s blood coming from your injury.\n   • You can’t walk.\n   • You can’t move your arms or legs.",
  },
];
